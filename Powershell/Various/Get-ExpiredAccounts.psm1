Function Get-ExpiredAccounts {
    ...
    if (isDateTime ($date)) {
        ...
        if ((get-date $date).Date -gt (get-date $now).Date) {
            do {
                Write-Host -BackgroundColor red "Gotcha Ya, Are You Trying To Cheat? Enter a date before Today!"
        ...
        Write-Host -BackgroundColor red "Incorrect Date Entered."
        $date = Read-Host -Prompt "Enter The Desired Date in the Following Format Day/Month/Year"
        if (isDateTime ($date)) {
            if ((get-date $date).Date -gt (get-date $now).Date) {
                do {
    ...
    $progressCounter = 0
    $totalAccounts = Search-ADAccount -AccountExpired -UsersOnly | where-Object {$_.accountexpirationdate -lt (get-date "$date")} | Measure-Object
    if ($totalAccounts.Count -gt 0) { 
        ...
        if ($action -eq "D") {
            ...
            $users = Search-ADAccount -AccountExpired -UsersOnly  | where-Object {$_.accountexpirationdate -lt (get-date "$date")} | Select-Object -expandproperty samaccountname
            ...
            Search-ADAccount -AccountExpired -UsersOnly | where-Object {$_.accountexpirationdate -lt (get-date "$date")} | Remove-AdUser 
        }
        elseif ($action -eq "M") {
            ...
            Search-ADAccount -AccountExpired -UsersOnly | where-Object {$_.accountexpirationdate -lt (get-date "$date")} | Disable-ADAccount 
        } 
        elseif ($action -eq "L") {
            Write-Host -BackgroundColor white -foregroundcolor black "Listing, Smart Choice"
            Search-ADAccount -AccountExpired -UsersOnly | where-Object {$_.accountexpirationdate -lt (get-date "$date")} | Select-Object SamAccountName, LastLogonDate, AccountExpirationDate 
